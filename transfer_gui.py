...
class ProAPI:
    """iMouse 专业版 HTTP API 封装 (端口9912)"""

    def __init__(self, host="127.0.0.1", port=9912):
    ...
    def _call(self, fun, data=None, timeout=None):
        """默认 timeout=None，保持与参考实现一致（无限等待）"""
        payload = {"fun": fun, "msgid": self._get_msgid(), "data": data or {}}
        resp = self._session.post(self.base_url, json=payload, timeout=timeout)
        return resp.json()

    def is_connected(self):
        try:
            r = self._call("get_device_list", timeout=5)
    ...
    def click(self, deviceid, x, y, button="left"):
        return self._call("click", {
            ...
            "x": int(x), "y": int(y), "time": 0
        })

    def swipe(self, deviceid, direction, length=0.6, sx=None, sy=None):
        data = {"deviceid": deviceid, "direction": direction, "length": length}
        if sx is not None:
            data["sx"] = sx
        if sy is not None:
            data["sy"] = sy
        return self._call("swipe", data)
    ...
    def send_key(self, deviceid, fn_key=None, key=None):
        ...
        if key:
            data["key"] = key
        return self._call("send_key", data)
    ...
    def screenshot(self, deviceid):
        """截图，返回 bytes 或 None"""
        r = self._call("get_device_screenshot", {
            "deviceid": deviceid, "isjpg": True
        }, timeout=20)
        if r.get("status") == 0:
            return _base64.b64decode(r["data"]["img"])
        return None

    def find_image(self, deviceid, img_b64, similarity=0.7):
        """找单张图，返回 [x, y] 或 None"""
        r = self._call("find_image", {
            "deviceid": deviceid, "img": img_b64, "similarity": similarity
        }, timeout=15)
        if r.get("status") == 0 and r.get("data", {}).get("code") == 0:
            return r["data"]["result"]
        return None

    def find_image_ex(self, deviceid, img_list, similarity=0.7):
        """从 img_list 中找任意一个，返回 [x, y] 或 None"""
        r = self._call("find_image_ex", {
            "deviceid": deviceid, "img_list": img_list,
            "similarity": similarity, "all": False
        }, timeout=15)
        if r.get("status") == 0:
            lst = r.get("data", {}).get("list", [])
            if lst and lst[0].get("code") == 0:
                return lst[0]["result"]
        return None

    def ocr(self, deviceid, rect=None):
        """全屏（或指定区域）OCR，返回 [{txt, confidence, result:[x,y]}]"""
        data = {"deviceid": deviceid}
        # rect 格式: [[top_left_x,top_left_y],[bot_left_x,bot_left_y],[top_right_x,top_right_y],[bot_right_x,bot_right_y]]
        data["rect"] = rect if rect else [[0, 0], [0, 1600], [700, 0], [700, 1600]]
        r = self._call("ocr", data, timeout=20)
        if r.get("status") == 0 and r.get("data", {}).get("code") == 0:
            return r["data"].get("list", [])
        return []

    def find_text(self, deviceid, text_list, similarity=0.7):
        """OCR全屏后按文字列表过滤，返回匹配的 [{txt, result:[x,y]}] 列表"""
        items = self.ocr(deviceid)
        matches = []
        for item in items:
    ...
    def shortcut(self, deviceid, shortcut_id, parameter='', outtime=30000, devlist=None):
        """执行快捷指令。严格对齐参考实现。"""
        data = {
            "deviceid": deviceid,
            "id": shortcut_id,
            "devlist": devlist or [],
            "parameter": _json.dumps(parameter) if isinstance(parameter, (dict, list)) else parameter,
            "outtime": outtime
        }
        return self._call("shortcut", data, timeout=None)

    def album_upload(self, deviceid, file_paths, album_name="Recents", outtime=60000):
        """上传到相册 - shortcut id=7，严格对齐参考"""
        return self.shortcut(deviceid, 7, {
            "name": album_name or "Recents",
            "list": file_paths or []
        }, outtime=outtime)

    def clipboard_set(self, deviceid, text):
        return self.shortcut(deviceid, 10, {"text": text})

    def exec_url(self, deviceid, url):
        return self.shortcut(deviceid, 13, {"url": url})

    def album_list(self, deviceid, num=20, date=""):
        r = self.shortcut(deviceid, 1, {"date": date, "num": num})
        if r.get("status") == 0:
            retdata = r.get("data", {}).get("retdata", "[]")
            try:
                return _json.loads(retdata) if isinstance(retdata, str) else retdata
            except Exception:
                return []
        return []

    def album_down(self, deviceid, file_names):
        return self.shortcut(deviceid, 2, file_names)

    def file_list(self, deviceid, path="/"):
        r = self.shortcut(deviceid, 4, {"path": path})
        if r.get("status") == 0:
            retdata = r.get("data", {}).get("retdata", "[]")
            try:
                return _json.loads(retdata) if isinstance(retdata, str) else retdata
    ...
    def file_down(self, deviceid, file_paths):
        return self.shortcut(deviceid, 5, file_paths)

    def file_upload(self, deviceid, file_paths, target_path="/", outtime=60000):
        """上传到文件系统 - shortcut id=8，严格对齐参考"""
        return self.shortcut(deviceid, 8, {
            "path": target_path,
            "list": file_paths or []
        }, outtime=outtime)
...
class DeviceInfo:
    ...
    def __init__(self, raw, deviceid):
        ...
        # 专业版字段: name=自定义名, username=设备名, device_name=型号
        self.name = (raw.get("name") or "").strip()              # 自定义名
        self.user_name = (raw.get("username") or "").strip()     # 设备名
        self.device_name = (raw.get("device_name") or "").strip()  # 型号（如 iPhone 11）
...
class TransferApp:
    def __init__(self, root):
        self.root = root
        self.root.title("iMouse Pro 自动化工具 V2.0")
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="iMouse Pro 自动化工具 V2.0",
    ...
    def _build_publish_tab(self):
        ...
        # 默认路径
        default_media = r"D:\iMouse\Shortcut\Media"
    ...
    def _auto_connect(self):
        self.log("正在连接 iMouse 专业版内核 (localhost:9912) ...", "info")
        self._set_status("连接中...", "#FFD54F")
        threading.Thread(target=self._connect_thread, daemon=True).start()

    def _connect_thread(self):
        try:
            self.pro_api = ProAPI(host="127.0.0.1")
            for i in range(15):
                time.sleep(1)
                if self.pro_api.is_connected():
                    self.connected = True
                    self.root.after(0, lambda: self._set_status("已连接", SUCCESS_COLOR))
                    self.root.after(0, lambda: self.log("已成功连接到 iMouse 专业版内核 (9912)", "ok"))
                    self.root.after(100, self._refresh_devices)
                    return
            self.root.after(0, lambda: self._set_status("连接超时", ERROR_COLOR))
            self.root.after(0, lambda: self.log("连接超时，请确认 iMouse 专业版内核已启动 (端口9912)", "err"))
    ...
    def _refresh_devices_thread(self):
        try:
            raw = self.pro_api.get_device_list()
            # 调试：打印第一台设备的原始字段
            if raw:
    ...
    def _find_and_click(self, deviceid, icon_keys, timeout=15, offset_x=0, offset_y=0):
        ...
        while time.time() - start < timeout:
            if self.stop_publish:
                return None
            try:
                result = self.pro_api.find_image_ex(deviceid, img_b64_list, similarity=0.7)
                if result:
                    x = result[0] + offset_x
                    y = result[1] + offset_y
                    self.pro_api.click(deviceid, x, y)
                    time.sleep(0.5)
                    return (x, y)
            except Exception:
                pass
    ...
    def _click_nth_thumbnail(self, deviceid, n, log_fn):
        ...
        from PIL import Image as PILImage
        try:
            img_bytes = self.pro_api.screenshot(deviceid)
            if img_bytes:
                img = PILImage.open(BytesIO(img_bytes)).convert('RGB')
                w, h = img.size
            else:
    ...
    def _find_image_in_gallery(self, deviceid, file_path, timeout=15):
        ...
        while time.time() - start < timeout:
            if self.stop_publish:
                return None
            try:
                result = self.pro_api.find_image(deviceid, img_b64, similarity=0.7)
                if result:
    ...
    def _publish_single_device(self, deviceid, dev_name, file_path="", music_url="",
        ...
        if content_type == "picture":
            ...
            # Step 7: 直接点击第 task_index 张图（不滚动）
            log(f"Step 7: 选择第 {task_index} 张图片...")
            nth_pos = self._click_nth_thumbnail(deviceid, task_index, log)
            if not nth_pos:
                log("  截图获取失败，使用固定网格位置", "warn")
                # 固定网格：390px宽，4列，grid_top≈118px
                col = (task_index - 1) % 4
                row = (task_index - 1) // 4
                x = col * 97 + 48
                y = 118 + row * 97 + 48
        ...
        # ═══════════ 视频发布流程 ═══════════
            ...
            # Step 6: 直接点击第 task_index 张视频（不滚动）
    ...
    def _upload_thread(self, devices):
        ...
        for dev_idx, dev in enumerate(devices):
            ...
            for i, fp in enumerate(files):
                ...
                try:
                    if mode == "album":
                        r = self.pro_api.album_upload(dev_id, [fp], album_name=target_path)
                    else:
                        r = self.pro_api.file_upload(dev_id, [fp], target_path=target_path or "/")
                    if r.get("status") == 0:
    ...
    def _list_remote_thread(self, dev):
        ...
        try:
            if mode == "album":
                file_list = self.pro_api.album_list(dev_id, num=num)
                self.album_files = file_list if file_list else []
            else:
                file_list = self.pro_api.file_list(dev_id, path=path or "/")
    ...
    def _download_thread(self, dev, selected_iids):
        ...
        try:
            if mode == "album":
                ...
                r = self.pro_api.album_down(dev_id, file_names)
            else:
                ...
                r = self.pro_api.file_down(dev_id, file_paths)
    ...
    def _do_update(self, new_version):
        ...
        try:
            ...
            with _zf.ZipFile(buf) as zf:
                # GitHub zip 解压后外层有一个文件夹 imouse-pro-tool-main/
                names = zf.namelist()
                if not names:
